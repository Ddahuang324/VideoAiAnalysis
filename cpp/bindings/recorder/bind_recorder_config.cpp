...
void bind_recorder_config(py::module& m) {
    ...
    py::class_<VideoEncoderConfig>(m, "VideoEncoderConfig", "视频编码器配置")
        .def(py::init<>(),
             "默认构造函数")
        .def_readwrite("output_file_path", &VideoEncoderConfig::outputFilePath,
                       "输出文件路径")
        .def_readwrite("width", &VideoEncoderConfig::width,
                       "视频宽度")
        .def_readwrite("height", &VideoEncoderConfig::height,
                       "视频高度")
        .def_readwrite("fps", &VideoEncoderConfig::fps,
                       "帧率")
        .def_readwrite("bitrate", &VideoEncoderConfig::bitrate,
                       "码率 (bps)")
        .def_readwrite("crf", &VideoEncoderConfig::crf,
                       "质量参数 (CRF, 0-51)")
    ...
    py::class_<AudioEncoderConfig>(m, "AudioEncoderConfig", "音频编码器配置")
        .def(py::init<>(),
             "默认构造函数")
        .def_readwrite("enabled", &AudioEncoderConfig::enabled,
                       "是否启用音频")
        .def_readwrite("sample_rate", &AudioEncoderConfig::sampleRate,
                       "采样率 (Hz)")
        .def_readwrite("channels", &AudioEncoderConfig::channels,
                       "通道数 (1=单声道, 2=立体声)")
        .def_readwrite("bitrate", &AudioEncoderConfig::bitrate,
                       "码率 (bps)")
        .def_readwrite("codec", &AudioEncoderConfig::codec,
                       "编码器名称 (aac, mp3, etc.)")
        .def("validate", &AudioEncoderConfig::validate,
             "验证配置有效性")
        .def("__repr__", [](const AudioEncoderConfig& c) {
            return "<AudioEncoderConfig " +
                   std::string(c.enabled ? "enabled" : "disabled") +
                   " " + std::to_string(c.sampleRate) + "Hz " +
                   std::to_string(c.channels) + "ch " + c.codec + ">";
        });
    ...
    py::class_<ZMQConfig>(m, "ZMQConfig", "ZMQ 通信配置")
        .def(py::init<>(),
             "默认构造函数")
        .def_readwrite("endpoint", &ZMQConfig::endpoint,
                       "ZMQ 端点地址 (如 tcp://*:5555)")
        .def_readwrite("timeout_ms", &ZMQConfig::timeoutMs,
                       "超时时间 (毫秒)")
        .def_readwrite("io_threads", &ZMQConfig::ioThreads,
                       "IO 线程数")
        .def("validate", &ZMQConfig::validate,
             "验证配置有效性")
        .def("__repr__", [](const ZMQConfig& c) {
            return "<ZMQConfig " + c.endpoint + " timeout=" +
                   std::to_string(c.timeoutMs) + "ms threads=" +
                   std::to_string(c.ioThreads) + ">";
        });
    ...
    py::class_<RecorderConfig>(m, "RecorderConfig", "录制器统一配置")
        .def(py::init<>(),
             "默认构造函数")
        .def_readwrite("zmq_publisher", &RecorderConfig::zmqPublisher,
                       "ZMQ 发布器配置")
        .def_readwrite("video", &RecorderConfig::video,
                       "视频编码配置")
        .def_readwrite("audio", &RecorderConfig::audio,
                       "音频编码配置")
        ...
        .def("save_to_file", &RecorderConfig::saveToFile,
             ...
             "参数:\n"
             "    filepath (str): 保存路径")
        .def("to_json", [](const RecorderConfig& c) {
            return c.toJson().dump();
        }, "将配置转换为 JSON 字符串")
        .def("from_json", [](RecorderConfig& c, const std::string& jsonStr) {
        ...
        .def("__repr__", [](const RecorderConfig& c) {
            return "<RecorderConfig video=" + std::to_string(c.video.width) + "x" +
                   std::to_string(c.video.height) + "@" +
                   std::to_string(c.video.fps) + "fps audio=" +
