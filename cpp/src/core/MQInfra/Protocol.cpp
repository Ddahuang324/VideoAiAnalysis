...
bool Protocol::sendFrameMessageZeroCopy(zmq::socket_t& socket, const FrameMessage& frame) {
    try {
        // Send Header
        socket.send(zmq::buffer(&frame.header, sizeof(frame.header)), zmq::send_flags::sndmore);

        zmq::message_t img_msg(const_cast<uint8_t*>(frame.image_data.data()),
                               frame.image_data.size(), nullptr, nullptr);
        socket.send(img_msg, zmq::send_flags::sndmore);

        socket.send(zmq::buffer(&frame.crc32, sizeof(frame.crc32)), zmq::send_flags::none);
